...
> **Status**: **[COMPLETED]**  
> **Key Architecture Upgrade**: Replaced pure-Python TF-IDF Vector Space Model (VSM) with Local ONNX Embeddings (`all-MiniLM-L6-v2`) & Persistent ChromaDB Vector Store.
...
### The Solution: Semantic RAG with Local ONNX Embeddings + ChromaDB
In Version 2 Phase 1, we upgraded the retrieval pipeline to **Semantic Vector Search**:
- **ONNX `all-MiniLM-L6-v2`**: Converts text into 384-dimensional dense vector embeddings that capture deep semantic relationships across security concepts.
...
- **100% Free & Offline Operations**: By using Chroma's built-in ONNX embedding function, the system calculates all vector representations locally on CPU without sending payload data to external APIs or requiring API key permissions.
- **Dual-Engine Architecture with Graceful Fallback**: If ChromaDB is unconfigured/offline, the retriever automatically falls back to our V1 TF-IDF engine, guaranteeing 100% application uptime.
...
├── chroma_store.py     # Persistent ChromaDB Client & Collection Manager (Local ONNX)
